...
RUN mkdir -p /out && \
    CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/bridgeyok-api ./apps/api/cmd/api && \
    CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/wsprobe ./apps/api/cmd/wsprobe && \
    CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/wscheck ./apps/api/cmd/wscheck && \
    CGO_ENABLED=0 GOBIN=/out go install -trimpath -ldflags="-s -w" -tags="no_clickhouse no_libsql no_mssql no_mysql no_sqlite3 no_vertica no_ydb" github.com/pressly/goose/v3/cmd/goose@v3.27.3
...
RUN apt-get update && \
    ...
    mkdir -p /workspace/bin /workspace/db/migrations /workspace/scripts && \
