...
export function SocialUsers({ viewerId, table, initialQuery = "", selection }: { selection?: { selectedIds: string[]; onSelect: (profile: Profile) => void }; initialQuery?: string; viewerId: string; table?: { id: string; inviteCode: string; disabled: boolean } }) {
  ...
  return <section className="social-users" aria-label={table ? "Undang pemain" : "Friends dan pencarian"}>
    ...
    {state.loading ? <p role="status">Memuat pemain…</p> : state.error ? <div role="alert"><p className="form-error">{state.error}</p><button type="button" onClick={reload}>Coba lagi</button></div> : state.profiles.length === 0 ? <p role="status">{query ? "Tidak ada pemain yang cocok." : friendsOnly ? "Belum ada Friends. Cari pemain dan saling follow untuk berteman." : "Belum ada pemain."}</p> : <ul className="social-list">{state.profiles.map(profile => <li key={profile.id}>
      ...
      {!selection && profile.friends ? <button type="button" onClick={() => setChatProfile(profile)}>Chat</button> : null}
      {selection ? <button type="button" disabled={selection.selectedIds.includes(profile.id)} onClick={() => selection.onSelect(profile)} aria-label={`Pilih ${profile.displayName}`}>{selection.selectedIds.includes(profile.id) ? "Terpilih" : "Pilih"}</button> : <FollowAction profile={profile} viewerId={viewerId} onChanged={reload} />}
