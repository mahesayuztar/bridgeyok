...
async function handle(request: Request, context: { params: Promise<{ path?: string[] }> }) {
  ...
  const allowed = /^(signup|login|logout|heartbeat|profile|users|invitations|chat|matches|history\/boards|matches\/[a-f0-9-]+(?:\/(?:ready|start|cancel))?|users\/[a-f0-9-]+\/follow|tables\/[a-f0-9-]+\/(participants|invites))?$/;
  ...
  try {
    ...
    const response = await fetch(new URL(`${path === "matches" || path.startsWith("matches/") || path === "history/boards" ? `/v1/${path}` : `/v1/account${path ? `/${path}` : ""}`}${new URL(request.url).search}`, ACCOUNT_API_URL), {
