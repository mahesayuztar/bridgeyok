...
async function handle(request: Request, context: { params: Promise<{ path?: string[] }> }) {
  ...
  try {
    ...
    const response = await fetch(new URL(`${path === "matches" || path.startsWith("matches/") || path === "history/boards" || path.startsWith("history/boards/") ? `/v1/${path}` : `/v1/account${path ? `/${path}` : ""}`}${new URL(request.url).search}`, ACCOUNT_API_URL), {
