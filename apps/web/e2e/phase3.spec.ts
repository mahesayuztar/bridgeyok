...
test("bot consensus follows human partners, recovers pending votes, and rejects bot pairs", async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await owner.getByRole("button", { name: "Buka menu Bot, kursi E, bot" }).click();
    await owner.getByRole("button", { name: "Keluarkan bot", exact: true }).click();
    await expect(owner.getByRole("button", { name: "Buka menu kursi kosong E" })).toBeVisible();
