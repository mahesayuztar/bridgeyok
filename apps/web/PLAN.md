...
- **Problem:** the previous ADR/code disabled claim and undo whenever any bot was seated; the requested partner-follow policy instead needs deterministic server responses and termination without softlock.
...
- **Completion gate:** ENG-03 PASS on 6 September 2026. The 288-case transition matrix, actor/repository recovery, protocol impersonation rejection, and full browser suite pass. Requests terminate after all required decisions; durable human approvals resume bot follow-up after hydration. ADR-007/ADR-008 explicitly supersede global bot exclusion. Evidence is recorded below.
...
ENG-03 consensus transition report: PASS — 6 September 2026. ADR-007/ADR-008 and OD-22 explicitly supersede blanket bot exclusion. Commits 9c11e6a, 965abf5, d1f7dfc, 1ffc02d, 2a7bc4e, 9a11113 cover policy, authoritative votes, recovery, client capability, zero-trick projection, and protocol impersonation rejection. Claim/undo transition matrix 288 cases covers requester seats, bot configurations, response order, human rejection, serialization, unchanged rejected games, exact undo restoration, and single terminal results. Actor hydration and uncached PostgreSQL integration verify pending approval recovery, duplicate bot delivery, and exactly one terminal event. API race suite, go vet, API lint (0 issues), web unit 49/49, contracts 3/3, typecheck, lint, and production build PASS. Uncached database integration PASS (133.834s); realtime integration PASS in isolation (3.820s) after the concurrent run exceeded an existing scripted frame deadline. Browser commits 6199303, 00d39e3, 1087d0b cover partner-follow accept/reject, mobile pending-request reload, bot-pair rejection, replacement/rejoin, zero-trick claim scoring, raw-frame privacy, and client vote-field exclusion. Viewport inspection pauses the browser clock to keep the five-second board result visible; the consensus scenario reuses a validated trick boundary during seat replacement. Final combined Playwright 4/4 PASS (4.9m): four-human regression 3.5m, bot consensus 1.0m, stale recovery and cross-tab leave. Desktop/tablet/mobile regressions cover 1920×1080 / 1024×768 / 768×1024 / 390×844 / 320×700; bot-specific screenshots reviewed at 1440×900 and 320×700 confirm accessible voting and recovered play controls. Screenshots: test-results/phase3-bot-consensus-follo-2a809-votes-and-rejects-bot-pairs/{bot-undo-pending-320x700,bot-pair-rejection-1440x900}.png. Final typecheck and lint also PASS after the browser test refinements.
```
...
## Draggable dialogs and miniature replay — 9 September 2026

- Dialog headers support mouse, touch, and keyboard movement, with viewport bounds. Coverage includes replay, score history, auction history, trick history, participant actions, leave confirmation, claim selection, and claim/undo requests.
...
- [x] P6-05: safe hidden input/motion, explicit board progression dan card geometry; commits `662a333`, `3122c23`, `111eed2`, browser empat akun dan session baseline lulus pada PostgreSQL 17 disposable.
...
P6-01–P6-05 PASS; P6-06–09 dan seluruh Objective of Done Phase 6 masih pending. Browser audit awal tetap dijelaskan di evidence.
