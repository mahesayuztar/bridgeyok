...
**P6-07 checkpoint — PASS, 24 September 2026:** history dipindahkan ke satu board feed lintas casual, bot, dan Team Match. `GET /v1/history/boards` memakai keyset cursor `(completed_at, board_id)`, page size 16 dengan maksimum 40, authorization berbasis board participation, lineup snapshot, signed perspective score, dan actual Team Match IMP. UI memakai infinite scroll dengan satu request aktif, de-duplication, stable selection, active score-sheet fallback tanpa full table GET, serta satu entitlement refresh ketika Team Match menjadi COMPLETE. Replay/analysis tetap memakai authorization endpoint existing dan menolak Team Match yang belum selesai. Evidence: history cursor unit tests, full Go test suite, web lint/typecheck/production build, contract tests, migration validation, dan generated-source regeneration lulus; dua warning `aria-description` pada score controls tetap pre-existing. Tidak ada deployment atau production migration dilakukan.
