...
> Status: Phase 0–2 selesai; Phase 3 berjalan; Objective GUX UX-01–UX-14, UX-G1, dan ENG-01–ENG-03 PASS; Phase 4 teknis selesai, review bridge independen pending; Phase 5 API/actor, UI, dan hardening lokal PASS; release gates pending; Phase 6 berjalan dengan P6-01–P6-07 PASS
...
- **Claim dan undo berbasis persetujuan tersedia pada empat seat terisi. Sesuai OD-22/ENG-03, bot mengikuti persetujuan partner manusia; pasangan bot-bot menolak. Pengajuan undo manusia juga menjadi persetujuan implisit bagi bot partnernya. Director adjustment UI, alert/convention system, tournament movement selain internal Team Match, ranking, chat, spectator, strategic/AI bot, voice/video, dan uang nyata bukan scope.** Claim hanya dapat diajukan pada batas trick dan undo hanya menargetkan aksi game terakhir milik pengaju.
...
### Objective GUX — Gameplay UX Reliability & Interaction Refactor — COMPLETE / UX-01–UX-14 PASS
...
ENG-02 **PASS pada 5 September 2026**. Navbar menyediakan ledger hasil board dan total pasangan dari proyeksi authoritative. Atribusi tetap mengikuti lineup awal board meski kursi atau pemain berubah; pengiriman ulang command tidak menggandakan skor, dan restart/refresh mempertahankan hasil. Unit, contract, race, PostgreSQL integration, build, serta Playwright 3/3 lulus dengan pemeriksaan desktop/tablet/mobile hingga 320 px, scroll keyboard, pointer nyata, dan privasi payload. Bukti lengkap berada di `apps/web/PLAN.md` bagian 23. ENG-03 mengikuti supersession eksplisit ADR-007/ADR-008 melalui OD-22.

ENG-03 **PASS pada 6 September 2026**. Claim/undo tersedia pada empat seat terisi dengan suara bot authoritative: mengikuti partner manusia, consent implisit pengaju undo bagi bot partnernya, dan penolakan deterministik pasangan bot-bot. Approval durable dipulihkan melalui actor/repository, respons palsu ditolak, dan claim nol trick diproyeksikan utuh. Sebanyak 288 skenario transisi, actor/repository recovery, race, integration, web unit 49/49, contracts 3/3, typecheck, lint, dan build lulus. Playwright lengkap 4/4 (4,9 menit) lulus untuk regresi empat pemain, consensus bot, reconnect, pergantian kursi, privasi payload, dan desktop/mobile. Bukti lengkap berada di `apps/web/PLAN.md` bagian 23.
...
**P6-03 checkpoint — PASS, 20 September 2026:** commits `90f3b33` dan `fcbbad2` memindahkan lobby/table ke shared `(app)` layout dan menambah `AppShell` yang menjaga satu table mount sebagai sibling slot workspace. Friends filter, Settings draft/status, scroll, Back/Forward, close, History route, loading/error boundary, serta explicit table/invite switch mengikuti lifecycle session tanpa memasukkan navigation ke reducer game. Browser PostgreSQL 17 membuktikan mount marker tetap sama dan navigation delta 0 socket/GET/subscribe/resume/takeover; refresh Settings pulih sekali di background, sedangkan simulated leave 503 mempertahankan meja lama. Unit 69/69, typecheck, lint tanpa error, build, persistence, leave, dan chat gates lulus; evidence rinci dicatat pada plan Phase 6.

**P6-04 checkpoint — PASS, 22 September 2026:** commits `d254dbc` dan `a6b1c40` membuat desktop rail icon-first dan mobile empat item menuju Table/Play, Friends, History, dan More. History memakai score sheet meja aktif serta daftar Team Match authorized; More memakai profile/settings, bantuan, dan preferensi suara yang sama dengan meja. Placeholder navigasi/Play dihapus, copy berulang dipangkas, logout failure tidak lagi membersihkan session. Browser PostgreSQL 17 dan web checks memverifikasi responsive navigation, invite, settings, logout, serta delta persistence tanpa socket/GET/subscribe/resume/takeover baru. Detail dan batas P6-07 dicatat di plan Phase 6.

**P6-07 checkpoint — PASS, 25 September 2026:** history dipindahkan ke satu board feed lintas casual, bot, dan Team Match. `GET /v1/history/boards` memakai keyset cursor `(completed_at, board_id)`, page size 16 dengan maksimum 40, authorization berbasis board participation, lineup snapshot, signed perspective score, actual Team Match IMP, dan personal learning label yang tersimpan per session. UI langsung dimulai dari satu `Recent boards` tanpa summary atau navigasi kembali ke meja; setiap row menggabungkan compact contract/result, menaruh label di sisi kanan, dan hanya menampilkan tanggal/jam kecil di bawahnya. Search server-side mendukung tanggal lokal, label, dan compact contract/result dengan debounce, cursor terikat query, satu request aktif, de-duplication, dan infinite scroll yang tetap efisien. Active score-sheet fallback tanpa full table GET dan satu entitlement refresh ketika Team Match menjadi COMPLETE tetap dipertahankan. Replay/analysis tetap memakai authorization endpoint existing dan menolak Team Match yang belum selesai. Evidence: history cursor/label unit tests, full Go test suite, web lint/typecheck/production build, contract tests, migration validation, dan generated-source regeneration lulus; dua warning `aria-description` pada score controls tetap pre-existing. Tidak ada deployment atau production migration dilakukan.
...
Status: **P6-01–P6-07 PASS; P6-05/P6-06 evidence sebelumnya tetap PASS; P6-08–09 pending**. Pending release gates Phase 4/5 tetap terpisah dan tidak dianggap selesai oleh checkpoint ini.
...
| OD-03 | Meja open ditutup dan seluruh session participant aktif di-expire setelah semua participant offline serta tidak ada meaningful activity selama **5 menit**. Completed result tetap direncanakan disimpan **90 hari**. | Heartbeat/presence tidak memperpanjang activity; expiry memakai revision fence, transaksi atomic, dan scan idempotent. Lihat ADR-012. |
| OD-04 | Owner disconnect tidak memindahkan ownership. Explicit leave memindahkan ownership ke participant aktif lain; bila owner terakhir keluar, meja ditutup. | Presence transport tidak menentukan membership atau ownership. Lihat ADR-011 dan ADR-012. |
...
| OD-20 | Play history diproyeksikan per recipient: Dummy mendapat trick 1–latest, participant lain hanya latest, dengan count publik terpisah. | Private snapshot tetap menyimpan history lengkap; raw snapshot/event non-Dummy tidak membawa earlier tricks; full deal setelah score tidak memperluas ordered history. Lihat ADR-013. |
| OD-21 | Score sheet satu meja memakai raw duplicate points per `table_id`, bukan IMP; identitas pasangan adalah dua occupant saat board dimulai dan total diorientasikan terhadap sisi pasangan pada setiap board. | Setiap row merujuk `board_id`/hasil `score_ns` durable; pertukaran seat tidak mengganti pair identity, substitusi mid-board tidak menulis ulang atribusi, dan IMP tetap hanya untuk Team Match. Lihat ADR-014. |
| OD-22 | Consensus bot mengikuti partner manusia melalui actor/repository authoritative; pasangan bot-bot menolak. | Request memerlukan empat seat terisi; manusia saja yang mengajukan, pengajuan undo menjadi consent implisit bagi bot partner, penolakan manusia langsung terminal, dan restart melanjutkan respons bot dari approval durable. Supersession eksplisit ADR-007/ADR-008 untuk ENG-03. |

Status OD-01 sampai OD-12: **CLOSED — accepted 29 Agustus 2026**. OD-13 sampai OD-16 mencatat refactor scope 30 Agustus 2026. OD-17 diterima 31 Agustus 2026 sebagai perubahan produk eksplisit dan menjadi baseline Phase 3–5. OD-18 diterima 1 September 2026 sebagai architecture/sequence baseline Objective GUX; implementation UX-01–UX-14 dan UX-G1 selesai 4 September 2026. OD-19 diterima 2 September 2026 dan menggantikan konfirmasi takeover manual dengan handoff otomatis pada connection terbaru. OD-20 diterima 4 September 2026 sebagai least-privilege play-history policy untuk ENG-01. OD-21 diterima 5 September 2026 sebagai semantics baseline raw duplicate Table Score Sheet untuk ENG-02. OD-22 diterima 5 September 2026 sebagai supersession larangan consensus global pada meja dengan bot.
